...
describe('Vuex Store 2025 Getters', () => {
  ...
  describe('FY 2025-2026 Tax Free Thresholds', () => {
    ...
    test('should calculate correct threshold for female taxpayer', () => {
      ...
      const threshold = store.getters.taxFreeThreshold2025;
      expect(threshold).toBe(425000); // Increased from 400k in 2024-25
    });
    ...
    test('should calculate correct threshold for senior citizen', () => {
      ...
      const threshold = store.getters.taxFreeThreshold2025;
      expect(threshold).toBe(425000); // Auto-detected senior (increased from 400k)
    });
    ...
    test('should calculate correct threshold for disabled person', () => {
      ...
      const threshold = store.getters.taxFreeThreshold2025;
      expect(threshold).toBe(500000); // Increased from 475k in 2024-25
    });
    ...
    test('should calculate correct threshold for freedom fighter', () => {
      ...
      const threshold = store.getters.taxFreeThreshold2025;
      expect(threshold).toBe(525000); // Increased from 500k in 2024-25
    });
    ...
    test('should calculate correct threshold for july warrior', () => {
      store.commit('updateTaxpayerProfile', {
      ...
      const threshold = store.getters.taxFreeThreshold2025;
      expect(threshold).toBe(375000); // july_warrior applies from FY 2026-27, falls back to general
    });
  });
  ...
  describe('FY 2025-2026 Tax Slabs', () => {
    ...
    test('should NOT include 5% tax bracket (removed in 2025-26)', () => {
      store.commit('updateTaxpayerProfile', {
        category: 'general',
        age: 30,
        location: 'dhaka'
  ...
  describe('FY 2025-2026 Investment Rebate', () => {
    ...
    test('should calculate investment rebate as min(3% of income, 15% of investment, 10L)', () => {
      ...
      // 3% of income = ~30,000; 15% of 100k = 15,000; cap = 1,000,000
      // Rebate = min(30000, 15000, 1000000) = 15,000
      const expected = Math.round(Math.min(taxableSalary * 0.03, 100000 * 0.15, 1000000));
      expect(investmentRebate).toBe(expected);
    });

    test('should be capped at 1,000,000 BDT (10 lakh)', () => {
      store.commit('changeSubsequentSalaries', { index: 0, value: 1000000 }); // Very high income
      store.commit('changeInvestment', { index: 0, value: 10000000 }); // Very high investment

      const investmentRebate = store.getters.investmentRebate2025;
      // 15% of 10M = 1.5M, but capped at 1M
      expect(investmentRebate).toBeLessThanOrEqual(1000000);
    });
  ...
  describe('Tax Comparison Getters', () => {
    ...
    test('should calculate tax savings when 2025 tax is lower', () => {
      ...
      const totalTax2025 = store.getters.totalTax2025;
      const taxDifference = totalTax2025 - totalTax2024;
      const taxSavings = taxDifference < 0 ? Math.abs(taxDifference) : 0;
      const additionalTax = taxDifference > 0 ? taxDifference : 0;
  ...
  describe('Integration with Existing Store', () => {
    test('should not interfere with existing 2024-25 calculations', () => {
      ...
      expect(threshold2024).toBe(350000); // 2024-25 unchanged
      expect(threshold2025).toBe(375000); // 2025-26 increased
